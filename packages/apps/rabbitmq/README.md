...
This chart adds TLS listeners for AMQP (port 5671) and the management HTTPS interface (port 15671) using cert-manager. By default it adds encrypted endpoints without closing the unencrypted ones — but it does change which endpoint clients are pointed at, and `tls.disableNonTLSListeners` can close the plaintext ones outright. Both are covered below.
